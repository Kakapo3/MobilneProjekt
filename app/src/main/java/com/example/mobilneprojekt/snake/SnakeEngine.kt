...
import com.example.mobilneprojekt.snake.SnakeEngine.Companion.BOARD_SIZE
...
class SnakeEngine(
    ...
    private fun generateFoodPosition(): Pair<Int, Int> {
        var a = Pair(Random.nextInt(0, BOARD_SIZE), Random.nextInt(0, BOARD_SIZE))
        while (mutableState.value.snake1.contains(a) || mutableStateOpponent?.value?.snake1?.contains(a) == true)
            a = Pair(Random.nextInt(0, BOARD_SIZE), Random.nextInt(0, BOARD_SIZE))
        return a
    }
